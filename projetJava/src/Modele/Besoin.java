...
public class Besoin {
    ...
    /**
     * Ajout d'une compétence au besoin de la mission et du nombre d'employés
     * nécessaire
     *
     * @param c Compétence
     * @param nbPers Nombre d'employé nécessaire pour cette compétence
     * @throws NbEmployeDepasseException si le quotat pour cette compétence est
     * dépassé
     */
    public void besoinParCompetence(Competence c, int nbPers) throws NbEmployeDepasseException {
        ...
        if (this.nbPersComp.containsKey(c)) {
            // Si diminution
            if (nbPers < this.nbPersComp.get(c)) {
                nbAct = nbAct - (this.nbPersComp.get(c) - nbPers);
            }
            // Si augmentation
            if (nbPers > this.nbPersComp.get(c)) {
                nbAct = nbAct + nbPers - this.nbPersComp.get(c);
            }
        }

        if (nbAct <= nbPersNecessaire) {
    ...
    public void removebesoinParCompetence(Competence c) {
    ...
    /**
     * Retourne la map contenant pour chaque compétence, le nombre d'employé a
     * affecter
     *
